---
title: "Activity assay of 3D HPLC fractions RNAseAlert assay for James"
author: "Jan Sklenar"
date: "22/5/2021"
...
## Goals	
Compare the activity profiles of purified aphid extract.Fractions are from 3D LC fractionation. 
...
ggplot(data = df_long) + 
  ...
  geom_point(aes(fill=RepNo), alpha=1/50) + 
...
Most of the data are not withing 95% the confidence intervals. 

***Summing up averages hides the differences in the data - many small values could completely hide the peaks! For that reason, it seems better to sum the replicates, then take average/smooth with the other replicates.*** 

***This is an MS technique; when we trust our signal we aquire we  can sum up enough spectra to suppress the noise. Then we can take several of such measurements.***
